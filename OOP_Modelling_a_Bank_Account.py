...
class BankAccount():
    def __init__(self, account_number, name, interest, balance):
        ...
        self.transaction = {"name": self.name, "type": "deposit",
                            "amount:": amount, "balance": self.balance, "date": today_date}
        self.transactions = []
    ...
    def withdraw(self, amount):
        ...
        if self.balance >= amount:
            ...
            self.balance -= amount
            self.transaction = {"name": self.name, "type": "Withdraw Successful",
                                "amount:": amount, "balance": self.balance, "date": today_date}
            self.transactions.append(self.transaction)
        else:
            print("Insufficient funds for ", self.name)
            self.transaction = {"name": self.name, "type": "Withdraw Failed",
                                "amount:": amount, "balance": self.balance, "date": today_date}
            self.transactions.append(self.transaction)
    ...
    def print_transactions_history(self):
        ...
        for i, transaction in enumerate(self.transactions, 1):
            print(
                f"{i}. Type: {transaction['type']}|amount: {transaction['amount:']}|Balance: {transaction ['balance']} x Date: {transaction['date']}")
...
class SavingAccount(BankAccount):
    ...
    def apply_monthly_interest(self):
        ...
        if today_date >= self.last_interest_date + relativedelta(months=1):
            ...
            self.monthly_interest_amount = (
                self.balance * (self.annual_rate/12))/100
...
Illy_Konto.check_balance()
Illy_Konto.last_interest_date = today_date - \
    relativedelta(months=6)  # Simulate last interest 6 months ago
